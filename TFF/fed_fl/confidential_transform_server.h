...
#ifndef CONFIDENTIAL_FEDERATED_COMPUTE_CONTAINERS_FED_FL_CONFIDENTIAL_TRANSFORM_SERVER_H_
#define CONFIDENTIAL_FEDERATED_COMPUTE_CONTAINERS_FED_FL_CONFIDENTIAL_TRANSFORM_SERVER_H_
...
// ConfidentialTransform service for Federated Learning. Executes the
// aggregation step of FL (e.g., FedAvg / FedOpt) inside a confidential
// environment.
class FedFlConfidentialTransform final
    : public confidential_federated_compute::ConfidentialTransformBase {
 public:
  FedFlConfidentialTransform(
 ...
 private:
  ...
  // Set the logged message config based on the initialization configuration.
    absl::Status SetAndValidateMessageFactory(
      const fcp::confidentialcompute::FedFlContainerInitializeConfiguration&
          config);
  //  Set the intrinsics based on the initialization configuration.
  absl::Status SetAndValidateIntrinsics(
      const fcp::confidentialcompute::FedFlContainerInitializeConfiguration&
          config);
  // Returns the configuration constraints for this worker. ...
  ...
  absl::Status ValidateConfigConstraints(
      const fcp::confidentialcompute::FedFlContainerConfigConstraints&
          config_constraints);
  ...
  // Key used to hash sensitive values in model checkpoints for FL.
...
#endif  // CONFIDENTIAL_FEDERATED_COMPUTE_CONTAINERS_FED_FL_CONFIDENTIAL_TRANSFORM_SERVER_H_
