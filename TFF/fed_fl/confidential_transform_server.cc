...
#include "containers/fed_fl/confidential_transform_server.h"
...
#include "containers/fed_fl/kms_session.h"
#include "containers/fed_fl/private_state.h"
#include "containers/session.h"
#include "fcp/base/monitoring.h"
#include "fcp/confidentialcompute/private_state.h"
#include "fcp/protos/confidentialcompute/blob_header.pb.h"
#include "fcp/protos/confidentialcompute/fed_fl_container_config.pb.h"
...
// #include "containers/fed_fl/ledger_session.h"
...
using ::fcp::confidentialcompute::FedFlContainerConfigConstraints;
using ::fcp::confidentialcompute::FedFlContainerInitializeConfiguration;
...
// changed OK (only rename)
absl::Status ValidateOuterDpParameters(const Intrinsic& intrinsic) {
  if (intrinsic.parameters.size() < 2) {
    return absl::InvalidArgumentError(
        "DP IntrinsicConfig must have at least epsilon and delta parameters.");
  }
...
// changed OK
FedFlConfidentialTransform::FedFlConfidentialTransform(
...
// changed OK
absl::Status FedFlConfidentialTransform::SetAndValidateIntrinsics(
    const FedFlContainerInitializeConfiguration& config) {
  ...
  // Validate the intrinsic config using TFF's built-in checks
  FCP_RETURN_IF_ERROR(
  ...
  // Ensure exactly one intrinsic
  FCP_RETURN_IF_ERROR(ValidateTopLevelIntrinsics(intrinsics));

  // NOTE: For FL, we do NOT apply SQL-specific DP checks here.
  // Any DP validation for FL aggregators (e.g., DPWeightedMean)
  // should be handled by TFF or KMS policy constraints.
  // ------------------------------------------------------------
  // const Intrinsic& fl_intrinsic = intrinsics.at(0);
  // if (fl_intrinsic.uri ==
  //         tensorflow_federated::aggregation::kDPGroupByUri ||
  //     fl_intrinsic.uri ==
  //         tensorflow_federated::aggregation::kDPTensorAggregatorBundleUri) {
  //   FCP_RETURN_IF_ERROR(ValidateFedSqlOuterDpParameters(fl_intrinsic));
  // }

  // Save final intrinsics
...
// changed OK
absl::StatusOr<Struct> FedFlConfidentialTransform::GetConfigConstraints(
    const FedFlContainerInitializeConfiguration& config) {
  ...
  // Always include the intrinsic URI
  (*config_properties.mutable_fields())["intrinsic_uri"].set_string_value(
      fl_intrinsic.uri);

  // NOTE:
  // For FL, we do NOT include epsilon/delta here.
  // DP validation (if any) will be handled by TFF or ValidateConfigConstraints.

  // Serialize destination node
...
// changed OK
absl::Status FedFlConfidentialTransform::ValidateConfigConstraints(
    const FedFlContainerConfigConstraints& config_constraints) {
  const std::vector<Intrinsic>* intrinsics;
  {
    absl::MutexLock l(&mutex_);
    if (intrinsics_ == std::nullopt) {
  ...
  if (!config_constraints.intrinsic_uris().empty()) {
    ...
    uri_is_valid = fl_intrinsic.uri == config_constraints.intrinsic_uri();
  }

  if (!uri_is_valid) {
    return absl::FailedPreconditionError(
      "Invalid intrinsic URI for configuration.");
  }

  // NOTE:
  // For FL, we do NOT enforce DP epsilon/delta constraints here yet.
  // Future FL-specific DP aggregators (e.g., DPWeightedMean) can add
  // additional checks here against config_constraints if needed.

  return absl::OkStatus();
}

absl::Status
FedFlConfidentialTransform::InitializeSessionInferenceConfiguration(
  ...
  if (inference_init_config.model_init_config_case() ==
    ...
    return absl::FailedPreconditionError(
        "When FedFlContainerInitializeConfiguration.inference_init_config is "
  ...
  for (const fcp::confidentialcompute::InferenceTask& inference_task :
    ...
    if (inference_task.inference_logic_case() ==
      ...
      return absl::FailedPreconditionError(
          "When FedFlContainerInitializeConfiguration.inference_init_config "
...
absl::Status FedFlConfidentialTransform::SetAndValidateMessageFactory(
    const FedFlContainerInitializeConfiguration& fed_fl_config) {
  absl::MutexLock l(&mutex_);
  if (message_factory_ != nullptr) {
    return absl::FailedPreconditionError(
        "SetAndValidateMessageFactory can only be called once.");
  }
  if (!fed_fl_config.logged_message_descriptor_set().empty() &&
      !fed_fl_config.logged_message_name().empty()) {
    FileDescriptorSet descriptor_set;
    if (!descriptor_set.ParseFromString(
            fed_fl_config.logged_message_descriptor_set())) {
      return absl::InvalidArgumentError(
    ...
    FCP_ASSIGN_OR_RETURN(
        ...
        MessageFactoryImpl::Create(descriptor_set,
                                   fed_fl_config.logged_message_name()));
  } else if (!fed_fl_config.logged_message_descriptor_set().empty() ||
             !fed_fl_config.logged_message_name().empty()) {
    return absl::InvalidArgumentError(
...
// changed OK
absl::Status FedFlConfidentialTransform::StreamInitializeTransformWithKms(
  ...
  // 1. Unpack configuration types
  FedFlContainerInitializeConfiguration fed_fl_config;
  if (!configuration.UnpackTo(&fed_fl_config)) {
    return absl::InvalidArgumentError(
        "FedFlContainerInitializeConfiguration cannot be unpacked.");
  }
  FedFlContainerConfigConstraints fed_fl_config_constraints;
  if (!config_constraints.UnpackTo(&fed_fl_config_constraints)) {
    return absl::InvalidArgumentError(
        "FedFlContainerConfigConstraints cannot be unpacked.");
  }

  // 2. Core initialization
  FCP_RETURN_IF_ERROR(SetAndValidateIntrinsics(fed_fl_config));
  FCP_RETURN_IF_ERROR(ValidateConfigConstraints(fed_fl_config_constraints));
  FCP_RETURN_IF_ERROR(SetAndValidateMessageFactory(fed_fl_config));

  // 3. Initialize private state (HPKE budget)
  FCP_RETURN_IF_ERROR(
      InitializePrivateState(fed_fl_config_constraints.access_budget()));

  // 4. FL does NOT use dp_windowing or dp_column_names from SQL DP
  //    So we REMOVE that entire block.

  // 5. Inference initialize (optional)
  if (fed_fl_config.has_inference_init_config()) {
    FCP_RETURN_IF_ERROR(InitializeSessionInferenceConfiguration(
        fed_fl_config.inference_init_config()));
  }

  // 6. Re-encryption settings
...
// changed OK (but actually didn't change anything -> no need)
absl::StatusOr<Struct> FedFlConfidentialTransform::StreamInitializeTransform(
    const InitializeRequest* request) {
  FedFlContainerInitializeConfiguration config;
  if (!request->configuration().UnpackTo(&config)) {
    return absl::InvalidArgumentError(
        "FedFlContainerInitializeConfiguration cannot be unpacked.");
  }

  // 1. Set intrinsic blueprint
  FCP_RETURN_IF_ERROR(SetAndValidateIntrinsics(config));

  // 2. Generate config constraints used by ledger/policy
  FCP_ASSIGN_OR_RETURN(Struct config_properties, GetConfigConstraints(config));

  // 3. Optional inference initialization
...
absl::Status FedFlConfidentialTransform::ReadWriteConfigurationRequest(
  ...
  if (write_configuration.has_first_request_metadata()) {
    ...
    // Create a new file.
    file_open_mode = std::ios::binary; // 用 binary mode "w"
    ...
    file_open_mode = std::ios::binary | std::ios::app; // append模式
  }
...
// 初始化與 KMS 金鑰使用相關的 PrivateState（包含 DP / access budget 資訊）
absl::Status FedFlConfidentialTransform::InitializePrivateState(
...
FedFlConfidentialTransform::CreateSession() {
  ...
  // 1. 拿到 aggregator blueprint（intrinsics）
  ...
  if (KmsEnabled()) {
    ...
    return std::make_unique<KmsFedFlSession>(
    ...
    return std::make_unique<FedFlSession>(
        std::move(aggregator), *intrinsics, inference_configuration_,
...
absl::StatusOr<std::string> FedFlConfidentialTransform::GetKeyId(
